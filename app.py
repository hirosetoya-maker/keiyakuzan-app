...
@st.fragment
def _view_table_fragment(supabase: Client) -> None:
    """読み取り専用テーブルをフラグメントとして描画。
    行クリックでダイアログが開き、保存後もページスクロール位置が保持される。"""
    ...
    filtered = df.copy()
    if f_query:
        search_cols = ["seller", "secondary_seller", "general_contractor",
                       "field_name", "contract_no", "memo"]
        mask = pd.Series(False, index=filtered.index)
        for col in search_cols:
            if col in filtered.columns:
                mask |= filtered[col].astype(str).str.contains(f_query, case=False, na=False)
        filtered = filtered[mask]
    if f_seller:
        filtered = filtered[filtered["seller"].str.contains(f_seller, case=False, na=False)]
    if f_secondary:
        filtered = filtered[filtered["secondary_seller"].str.contains(f_secondary, case=False, na=False)]
    if f_contractor:
        filtered = filtered[filtered["general_contractor"].str.contains(f_contractor, case=False, na=False)]
    if f_field:
        filtered = filtered[filtered["field_name"].str.contains(f_field, case=False, na=False)]
    filtered = (
        filtered
...
def page_contracts() -> None:
    ...
    # 読み取り専用テーブル（行クリックでダイアログ編集・フラグメントでページスクロール保持）
    _view_table_fragment(supabase)
...
def _detail_dialog(row: dict, supabase: Client) -> None:
    """契約数量・単価・備考・JV・契約日・着工日をまとめて編集するダイアログ"""
    st.markdown(f"**{row['field_name']}**　（契約NO: {row['contract_no']}）")

    qty_raw = pd.to_numeric(row.get("contract_qty"), errors="coerce")
    new_qty = st.number_input(
        "契約数量（m³）",
        min_value=0.0, step=0.5,
        value=float(qty_raw) if pd.notna(qty_raw) else None,
        placeholder="未入力",
    )
    ...
    new_memo = st.text_input("備考", value=row.get("memo") or "")
    ...
    if st.button("💾 保存する", type="primary", use_container_width=True):
        try:
            supabase.table("contracts").update({
                "contract_qty":  float(new_qty) if new_qty is not None else None,
